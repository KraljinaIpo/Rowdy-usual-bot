...
module.exports.run = async (bot, message, args) => {
    if(!args[0]) return message.channel.send("USAGE: %warn <user> <reason>");
    let rUser = message.guild.member(message.mentions.users.first() || message.guild.members.get(args[0]));
    
    if(!message.member.hasPermission("MANAGE_MEMBERS")) return message.reply("You can't do this pal!");
...
if(warns[wUser.id].warns == 5){
    ...
    setTimeout(function(){
        ...
        message.reply(`The ${wUser} has been unmuted!`)
    }, ms(mutetime))
}
